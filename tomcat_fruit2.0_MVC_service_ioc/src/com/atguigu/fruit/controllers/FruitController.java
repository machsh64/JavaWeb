...
public class FruitController {
    ...
    private String index(String opera,String keyValue,Integer pageNo,HttpServletRequest request) {
        ...
        List<Fruit> fruitList = fruitService.getFruitList(keyValue, pageNo);   //将pageNo赋值给list
        //保存到session作用域
        session.setAttribute("fruitList", fruitList);

        int params = fruitService.paramsQuery(keyValue);//数据库中的总页数  每页5行数据
    ...
    private String update(Integer fid,String fname,Integer price,Integer fcount,String remark){
        //3，执行更新
        fruitService.update(new Fruit(fid, fname, price, fcount, remark));
